...
func _draw_wing_gates() -> void:
	var ordered_wings := _hall_tree.get_ordered_wings()
...
func _draw_resonance_links() -> void:
	for resonance in _hall_tree.resonances:
		...
		if _progression:
			var discovered_list := _progression.get_discovered_resonances()
		...
		var resonance_line := Line2D.new()
		resonance_line.name = "Resonance_%s_%s" % [h1, h2]

		# Create a gentle curve between the two points
		var mid := (pos1 + pos2) / 2.0
		var offset_x := 80.0  # Curve offset to the right
		var curve_mid := Vector2(mid.x + offset_x, mid.y)
		var steps := 12
		for s in range(steps + 1):
			var t := float(s) / float(steps)
			# Quadratic Bezier: P = (1-t)^2*P0 + 2*(1-t)*t*PC + t^2*P1
			var p := (1.0 - t) * (1.0 - t) * pos1 + 2.0 * (1.0 - t) * t * curve_mid + t * t * pos2
...
func _animate_wing_unlock(wing_id: String) -> void:
	...
	var timer := get_tree().create_timer(WING_UNLOCK_DURATION + 0.5)
...
func _refresh_gate_visuals() -> void:
	...
	for i in range(1, ordered_wings.size()):
		...
		var gate_node := _gate_canvas.get_node_or_null("Gate_%s_%s" % [prev_wing.id, wing.id])
		if gate_node and gate_node is Line2D:
			gate_node.default_color = gate_color

		var label_node := _gate_canvas.get_node_or_null("GateLabel_%s" % wing.id)
...
func _get_prev_wing_id(wing_id: String) -> String:
	var wing := _hall_tree.get_wing(wing_id)
	if wing == null:
		return ""
